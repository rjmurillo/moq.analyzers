...
[InProcess]
[MemoryDiagnoser]
[BenchmarkCategory("Moq1600")]
public class Moq1600Benchmarks
{
...
public abstract class BaseClass{index}
{{
...
{{
    private void Test()
    {{
        var mock = new Mock<BaseClass{index}>();
        mock.Protected().Setup<bool>(""Foo"", It.IsAny<string>()).Returns(true);
        _ = new BaseClass{index}(); // Add an expression that looks similar but does not match
    }}
}}
");
        }
        ...
        (BaselineCompilation, TestCompilation) =
            BenchmarkCSharpCompilationFactory.CreateAsync<ProtectedSetupShouldUseItExprAnalyzer>(sources, referenceAssemblies)
            .GetAwaiter()
            .GetResult();
    }

    [Benchmark]
    public async Task Moq1600WithDiagnostics()
    {
    ...
    public async Task Moq1600Baseline()
    {
        ImmutableArray<Diagnostic> diagnostics =
