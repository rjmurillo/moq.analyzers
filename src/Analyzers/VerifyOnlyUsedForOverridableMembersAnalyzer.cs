...
public class VerifyOnlyUsedForOverridableMembersAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeInvocation(OperationAnalysisContext context)
    {
        ...
        // 1. Check if the invoked method is a Moq Verify method
        if (!targetMethod.IsMoqVerifyMethod(knownSymbols))
        {
            return;
        }

        // 2. Attempt to locate the member reference from the Verify expression argument.
    ...
    private static ISymbol? TryGetMockedMemberSymbol(IInvocationOperation moqVerifyInvocation)
    {
        ...
        //
        // In almost all Moq verifies, the first argument is the expression (lambda) to be analyzed.
