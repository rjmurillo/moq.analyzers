...
public class SetupShouldNotIncludeAsyncResultAnalyzer : DiagnosticAnalyzer
{
    ...
    {
        // This rule only applies to versions of Moq earlier than 4.16
        ImmutableArray<INamedTypeSymbol> mockTypes = context.Compilation.GetMoqMock();
        if (!mockTypes.IsEmpty)
        {
            INamedTypeSymbol mockTypeSymbol = mockTypes[0];
            IAssemblySymbol? assemblySymbol = mockTypeSymbol.ContainingAssembly;
            AssemblyIdentity identity = assemblySymbol.Identity;
            Version version = identity.Version;
            Version minimalVersion = new(4, 16);
