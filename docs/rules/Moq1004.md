...
Mocking `ILogger` or `ILogger<T>` is unnecessary and fragile. Use `NullLogger.Instance` (for `ILogger`)
or `NullLogger<T>.Instance` (for `ILogger<T>`) for tests that ignore logging, or `FakeLogger` from
`Microsoft.Extensions.Diagnostics.Testing` for tests that verify log output.
