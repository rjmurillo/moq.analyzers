...
- **IMP-003**: PerfDiff treats benchmark set mismatches, empty benchmark output, duplicate benchmark names, null JSON, single-sample measurements, and zero-operation measurements as comparison failures. It detects relative regressions through BenchmarkDotNet statistics, median and mean ratio gates, and absolute budgets. Mean-ratio and P95-ratio gates first exclude unstable per-benchmark changes whose delta is at or below the 0.5ms absolute floor or within two combined standard deviations of the baseline and diff runs. If standard deviation data is unavailable, those gates fall back to the 0.5ms absolute floor alone. They then fail only when the worse-set geometric mean is greater than the 5% aggregate ratio threshold. Individual benchmark changes remain diagnostic log entries. Absolute P95 (`250ms`) and mean (`100ms`) budgets report a regression only when the new run crosses from at-or-under budget to over budget. Existing over-budget benchmarks do not block CI unless they newly regress under another strategy. When CI fails due to a threshold breach, contributors should (a) verify the regression is real by running locally, (b) optimize if real, or (c) update the baseline if the regression is acceptable.
