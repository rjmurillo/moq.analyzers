...
- Phantom symbols still phantom: `grep -n "Moq.Language.IReturns" src/Common/WellKnown/MoqKnownSymbols.cs` then confirm absence in Moq metadata (`dotnet-inspect member "IReturns" --package Moq@4.18.4 --all`, pinning the `@version` and re-running `@4.8.2` since unpinned inspects only the latest release, or byte-scan the package DLL for a null-terminated `` IReturns` `` type name).
