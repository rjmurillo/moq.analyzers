...
| Rule behavior change (severity, category, span, logic) | Existing analyzer + its tests | Entry in `AnalyzerReleases.Unshipped.md` (see promotion section — never touch Shipped); regression tests pinning old-behavior cases that must still pass; span discipline (non-negotiables table); Moq-version compatibility note in PR body |
...
| 2 | **`AnalyzerReleases.Shipped.md` is immutable** outside release promotion. All rule additions AND changes to already-shipped rules go in `Unshipped.md`. | Shipped is the historical record consumed by Roslyn's release-tracking analyzer; rewriting it lies about what past package versions contained. | `.github/copilot-instructions.md:552`: "Do not modify AnalyzerReleases.Shipped.md. This file is an immutable record of past releases." Prior mix-up: swapped file names, issue #983. |
