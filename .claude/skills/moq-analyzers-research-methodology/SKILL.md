...
description: Applies the discipline that turns a hunch into an accepted result in moq.analyzers — the evidence bar (one mechanism must explain ALL observations including negatives), assigned adversarial refutation (the author never gets to be the only tester), predict-numbers-before-running for perf and FP work, the issue-to-explainer-to-PR idea lifecycle, and experiment hygiene. Load this when proposing a new analyzer idea or fix hypothesis, designing an experiment or corpus study, reviewing whether a claimed root cause is real, deciding how to write up or retire an investigation, or when someone says "I think I found the cause", "let's try", "this should be faster", or "can we delete this rule". Do NOT load it for executing an already-planned fix (moq-analyzers-rule-lifecycle for new rules, moq-analyzers-fp-convergence-campaign for the correctness backlog), single-claim verification recipes (moq-analyzers-proof-toolkit), picking which open problem to attack (moq-analyzers-research-frontier), or the raw incident chronicle (moq-analyzers-failure-archaeology).
---
