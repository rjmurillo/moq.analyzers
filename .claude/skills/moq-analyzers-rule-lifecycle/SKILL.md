...
**Known discrepancy — this section owns the call (2026-07-02):** the live `AnalyzerReleases.Unshipped.md` currently contains a `### Changed Rules` section (14 recategorized rules with old/new columns), which conflicts with the "only New Rules" instruction above. The upstream Roslyn release-tracking format accepts both shapes, so the build passes either way. Default: **follow the live file's current layout** — the `Changed Rules` section has been accepted on `main` since PR #1087, and `moq-analyzers-docs-and-writing` flags the copilot-instructions line as stale — but state the choice explicitly in the PR (never resolve the divergence silently, and never restructure the file in an unrelated PR). `moq-analyzers-change-control` defers to this section for the section-shape question.
